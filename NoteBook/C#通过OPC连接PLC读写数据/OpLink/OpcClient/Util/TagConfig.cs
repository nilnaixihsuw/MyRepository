...
namespace OpcClient
{
    ...
    public static class TagConfig
    {
        ...
        /// <param name="blockName"></param>
        /// <returns></returns>
        public static List<T> QueryTagsByBlockName<T>(string groupName, string blockName) where T : new()
        {
            XElement xGroups = XElement.Load(Path);
            XElement xGroup = xGroups.Elements("Group").Where(p => p.Attribute("Name").Value == groupName).FirstOrDefault();
            XElement xBlock = xGroup.Elements("Block").Where(p => p.Attribute("Name").Value == blockName).FirstOrDefault();

            return XmlToCollectionByBlock<T>(xBlock);
        }
        ...
        /// <param name="blockName"></param>
        /// <returns></returns>
        public static List<T> QueryTagsByGroupName<T>(string groupName) where T : new()
        {
            XElement xGroups = XElement.Load(Path);
            XElement xGroup = xGroups.Elements("Group").Where(p => p.Attribute("Name").Value == groupName).FirstOrDefault();
            return XmlToCollectionByGroup<T>(xGroup);
        }
        #endregion
        #region 删除
        /// <summary>
        ...
        {            
            ...
            foreach (var item in TCollection)
            {
                ...
                foreach (var property in type.GetProperties())
                    //只搜索带有Description特性,值为Save的属性 
                    if (property.DescriptionEqual("Save"))
                    {                        
        ...
        /// <summary>
        /// 判断MemberInfo的公共属性是否带有Description特性，并指定其内容为value
        /// </summary>
        /// <param name="member">无数据</param>
        /// <param name="value">描述内容</param>
        /// <returns></returns>
        public static bool DescriptionEqual(this MemberInfo member, string value)
        {
            if (member.IsDefined(typeof(DescriptionAttribute), true))
            {
                Object[] objs = member.GetCustomAttributes(typeof(DescriptionAttribute), false);

                DescriptionAttribute da = (DescriptionAttribute)objs[0];
                if (da.Description == value)
                {
                    return true;
                }
                else
        ...
        /// <param name="TCollection">泛型集合</param>
        /// <returns>集合的XML格式字符串</returns>
        public static List<T> XmlToCollectionByBlock<T>(XElement xBlock) where T:new()
        {
        ...
        /// <param name="TCollection">泛型集合</param>
        /// <returns>集合的XML格式字符串</returns>
        public static List<T> XmlToCollectionByGroup<T>(XElement xGroup) where T : new()
        {
            List<T> list = new List<T>();
            //获取泛型的具体类型
            Type type = typeof(T);
            ...
            foreach (XElement block in xGroup.Elements("Block"))
            {
               var listBlock = XmlToCollectionByBlock<T>(block);
                //把属性数组添加到元素中
               list.AddRange(listBlock);
            }
            return list;
        }
        /// <summary>
        ...
        {
            //读取配置文件
            IniReader.Instance.Load();
            return IniReader.Instance.G_TAGPATH;
        }
        #endregion
        /// <summary>
        /// 查询xml配置文件中的tags
