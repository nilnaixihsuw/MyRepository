    ...
    public class Messager
    {
        ...
        public static void SendMessage(DBContextBase dbContext, Messager CurrentMessager)
        {
            ...
            foreach (var entity in modifiedEntries)
            {
                ...
                //保存到当前待发送队列
                EntityListMerge(CurrentMessager.ModifiedEntityList, messageEntity);
                CurrentMessager.EntityNameList.Add(messageEntity.EntityName);
                CurrentMessager.IsPublish = true;
            }
        ...
        private static void EntityListMerge(List<MessageEntity> list, MessageEntity newEntity)
        {
            if (newEntity.Type == OperationType.MODIFIED)//实体修改
            {
                if (list.FirstOrDefault(m => m.EntityName.Equals(newEntity.EntityName)) is MessageEntity existEntity 
                    && existEntity.KeyNameList.Count == newEntity.KeyNameList.Count)
                {
                    foreach (var keyName in newEntity.KeyNameList)
                    {
                        if (existEntity.KeyNameList.Contains(keyName) &&
                            existEntity.CurrentValues[keyName].ToStringEx().Equals(newEntity.CurrentValues[keyName].ToStringEx()))
                        {
                            continue;
                        }
                        else
                    ...
                    //是同一条数据则进行合并
                    foreach (var item in newEntity.CurrentValues)
                    {
                        if (existEntity.CurrentValues.ContainsKey(item.Key))
                        {
                            if (!existEntity.CurrentValues[item.Key].ToStringEx().Equals(item.Value.ToStringEx()))
                            {
                                //更新当前值
                                existEntity.CurrentValues[item.Key] = item.Value;
                            }
                        }
                        else
                        {
                            //记录当前值
                            existEntity.CurrentValues.Add(item.Key, item.Value);
                        }
                    }
                }
                else
