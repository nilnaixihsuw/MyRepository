...
namespace Mediinfo.Infrastructure.Core.MessageQueue
{
    ...
    public class RabitMQChannel : IMessageClient, IDbMessageClient, IUserMessageClient
    {
        ...
        public void RePublish(Messager messageQueue)
        {
            try
            {
                if (channel == null) return;
                ...
                Dictionary<string, object> aHeader = new Dictionary<string, object>();
                aHeader.Add("moKuaiMc", messageQueue.MoKuaiMc);
                aHeader.Add("yeWuMc", messageQueue.YeWuMc);
                aHeader.Add("caoZuoMc", messageQueue.CaoZuoMc);
                foreach (var item in messageQueue.EntityNameList)
                {
                    if (string.IsNullOrEmpty(item.Trim()))
                        continue;
                ...
                var policy = Policy
                      ...
                      {
                          Enterprise.Log.LogHelper.Intance.Warn("超时预警", "服务:" + messageQueue.MoKuaiMc + "，DB消息重发超时",
                              "服务:" + messageQueue.MoKuaiMc + "，controller：" + messageQueue.YeWuMc + "，action：" + messageQueue.CaoZuoMc + "，发送DB消息超时" + ex.ToString() + "");
                      });
            ...
            {
                LogHelper.Intance.Error("消息队列", "DB消息重发失败",
                    "服务:" + messageQueue.MoKuaiMc + "，controller：" + messageQueue.YeWuMc + "，action：" + messageQueue.CaoZuoMc + "，异常原因: " + ex.ToString());
            }
        }
