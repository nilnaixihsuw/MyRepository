    ...
    public abstract class UnitOfWorkBase : IUnitOfWork, IRepositoryContext
    {
        ...
        public UnitOfWorkBase(ContextCache contextCache, DBContextBase dbcontext)
        {
            ...
            dbContext.Database.Log = (sql) =>
            {
                if (string.IsNullOrEmpty(sql) == false)
                {
                    ...
                    // 判断当前是否是 Opened connection at 2020/12/24 15:34:52 +08:00
                    if (sql.ToUpper().StartsWith("OPENED CONNECTION"))
                    {
                        /*
                            UnitOfWork at 2020/12/24 15:34:52
                            Opened connection at 2020/12/24 15:34:52 +08:00
                            connection at HIS6
                         */
                        stringBuilder.AppendLine("Unit Of Work at " + CreateTime.ToString("yyyy-MM-dd HH:mm:ss.fff")).AppendLine();
                        stringBuilder.AppendLine(sql);
                        stringBuilder.AppendLine("connection at " + dbcontext.GetDbSchemaName());
                    }

                    string sqlUpper = sql.ToUpper();
                    if (sqlUpper.Contains("INSERT") ||
                        sqlUpper.Contains("DELETE") ||
                        sqlUpper.Contains("UPDATE") ||
                        sqlUpper.Contains("SELECT"))
                    {
                        stringBuilder.AppendLine("Started at " + DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss.fff")).AppendLine();
                        stringBuilder.AppendLine(sql);
                    }
        ...
        public IEnumerable<TEntity> SqlQuery<TEntity>(string sql, Dictionary<string, object> parameters) where TEntity : class
        {
            ...
            parameters.ToList().ForEach(o =>
            {
                ...
                if (o.Value is DateTime || o.Value is DateTime?)
                {
        ...
        {
            return GetCanShu(yingyongId, canShuId, defaultValue, false);
        }
        ...
        /// <returns>返回：参数值</returns>
        public virtual string GetCanShu(string yingyongId, string canShuId, string defaultValue, bool realtime)
        {
            // 判断参数是否为空，如果为空则直接返回错误
            if (String.IsNullOrWhiteSpace(canShuId))
                throw new DomainException("参数ID为空！");

            // 判断应用ID是否为空，如果应用ID为空则直接取"00"
            if (String.IsNullOrWhiteSpace(yingyongId))
                yingyongId = "00";

            // 拼接参数Key
            string cacheKey = string.Format("{0}-{1}-{2}", "canshu", yingyongId, canShuId);
            // 判断是否实时取数据库
            if (!realtime)
            {
                // 检索缓存中是否存在该缓存，如果存在该缓存则直接返回缓存值
                if (this.ExistInCache<string>(cacheKey))
                {
                    return this.GetFromCache<string>(cacheKey);
                }
            }

            // 如果未检索到缓存或者要求实时取数据库则执行下面逻辑
            var canShuList = this.SqlQuery<CanShuDTO>("select yingyongid, canshuid, canshuzhi from gy_canshu where canshuid = :canshuid", canShuId);
            var canShu = canShuList.FirstOrDefault(c => c.YINGYONGID == yingyongId);

            // 如果从数据库中取到的参数值不为空时更新缓存并返回结果
            if (canShu != null)
            {
                PutToCache<string>(cacheKey, canShu.CANSHUZHI);
                return canShu.CANSHUZHI;
            }

            // 如果从数据库中取到的参数值不为空那么就取应用前两位的参数，如: 如果"0401"应用下的参数值为空那么就取"04"下面的
            canShu = canShuList.FirstOrDefault(c => c.YINGYONGID == yingyongId.Substring(0, 2));
            ...
            // 如果应用下的参数值都为空时则直接取"00"下面的全局参数
            canShu = canShuList.FirstOrDefault(c => c.YINGYONGID == "00");
            ...
            // 如果数据库中未找到改参数则直接将默认值返回并放入缓存
            PutToCache<string>(cacheKey, defaultValue);
            return defaultValue;
        }
