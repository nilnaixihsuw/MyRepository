...
namespace Mediinfo.Infrastructure.Core.Filter
{
    ...
    public class ApiActionFilterAttribute : ActionFilterAttribute
    {
        ...
        /// ApiActionFilter触发时间
        /// </summary>
        private DateTime _ActionExecutingTime;
        ...
        {
            _ActionExecutingTime = DateTime.Now;
        ...
        {
            //如果有异常抛出，跳过此处逻辑，在ApiExceptionFilterAttribute的OnException方法中会记录相关日志
                ...
                try
                {
                    ...
                    for (int i = 0; i < unitOfWorkDict.Values.Count; i++)
                    {
                        ...
                        try
                        {
                            ...
                            {
                                if (_ActionExecutingTime != null)
                                {
                                    sqlLogText.AppendLine("api action filter executing " + _ActionExecutingTime.ToString("yyyy-MM-dd HH:mm:ss.fff"));
                                    sqlLogText.AppendLine();
                                }
