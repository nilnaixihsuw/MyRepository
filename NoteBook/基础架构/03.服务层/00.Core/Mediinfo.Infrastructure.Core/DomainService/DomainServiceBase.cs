...
namespace Mediinfo.Infrastructure.Core.DomainService
{
    ...
    public abstract class DomainServiceBase : ICanShu
    {
        ...
        /// <returns>返回：参数值</returns>
        public string GetCanShu(string yingyongId, string canShuId, string defaultValue, bool realtime)
        {
            return ((IRepositoryContext)UnitOfWork).GetCanShu(yingyongId, canShuId, defaultValue, false);
        }
    }
}
