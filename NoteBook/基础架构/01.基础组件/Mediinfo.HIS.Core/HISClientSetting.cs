...
namespace Mediinfo.HIS.Core
{
    ...
    public class HISClientSetting : HISSetting
    {
        ...
        private static HISClientSetting _Zxtinstance = null;
        ...
        /// </summary>
        /// <returns></returns>
        public static HISClientSetting LoadNew(string ZxtPath)
        {
            if (!Directory.Exists(ZxtPath))
                Directory.CreateDirectory(ZxtPath);
            if (_Zxtinstance == null)
                _Zxtinstance = Load<HISClientSetting>(ZxtPath + @"\HISClientSetting.xml");
            return _Zxtinstance;
        }
        ...
        /// <summary>
        /// 用户名输入框是否自动写入最后一次登录工号信息
        /// </summary>
        [XmlElement(ElementName = "是否写入最后一次登录信息")]
        public bool IsAutoWriteLastLoginInfo { get; set; } = true;
    }
