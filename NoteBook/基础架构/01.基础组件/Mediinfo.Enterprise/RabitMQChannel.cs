...
namespace Mediinfo.Enterprise
{
    ...
    public class RabitMQChannel : IDisposable
    {
        ...
        /// 单例
        /// </summary>
        public static RabitMQChannel Channel => new RabitMQChannel("/");

        public static RabitMQChannel UserChannel => new RabitMQChannel("/user");

        public static RabitMQChannel DbChannel => new RabitMQChannel("/db");
        ...
        public EventingBasicConsumer HandlerMessage(string queueName)
        {
            try
            {
                ...
                channel.BasicConsume("USERMSG-" + queueName.Trim(), true, consumer);
                return consumer;
            }
            catch (Exception ex)
            {
