...
namespace Mediinfo.Enterprise.Config
{
    ...
    public class MediinfoConfig
    {
        ...
        /// <returns></returns>
        public static string GetXmlNodeValue(string filePath,string node)
        {
            if (!configures.ContainsKey(filePath))
            {
                lock (_lock)
                {
                    string config = IOHelper.Read(filePath);
                    // 如果未找到文件则返回空
                    if (string.IsNullOrWhiteSpace(config))
                        return "";
                    XmlDocument xml = new XmlDocument();
                    ...
                    // 获得xml的根节点
                    var configure = xml.DocumentElement;
                    configures.Add(filePath,configure);
                }
            }
            var snode = configures[filePath].SelectSingleNode(node);
            if (snode != null)
            {
                return snode.InnerText;
            }
