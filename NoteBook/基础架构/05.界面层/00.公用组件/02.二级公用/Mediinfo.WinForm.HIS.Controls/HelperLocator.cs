...
namespace Mediinfo.WinForm.HIS.Controls
{
    ...
    public class HelperLocator : IServiceProvider
    {
        private readonly IContainer _container;
        private static readonly HelperLocator _instance = new HelperLocator();
        ...
        private HelperLocator()
        {
            try
            {
                ...
                var types = assemblys.SelectMany(a => a.GetTypes().Where(t => t.GetInterfaces().Contains(typeof(IHelperInterface)))).ToArray();

                var impls = types.Where(t => !t.IsInterface).ToList();

                foreach (var impl in impls)
                {
                    var services = impl.GetInterfaces().Where(m => m != typeof(IHelperInterface));
        ...
        public static HelperLocator Instance
        {
